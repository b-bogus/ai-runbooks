...
**Project Goal:** To create a comprehensive set of context files within the `rules_bank` directory, improving the LLM Agent's understanding of the environment, tools, policies, and relevant threats.
...
*   **Tasks:**
    1.  **Create `governance_overview.md`**:
        ...
        *   Reference key policies (linking to where they might exist if not directly in `rules_bank`).
...
*   **Tasks (Ongoing/Iterative):**
    ...
    *   **Rationale:** Ensures runbooks remain effective, reflect real-world conditions, incorporate lessons learned (like tool errors), and leverage the full context available within the `rules_bank` directory.
